...
Ниже — краткая справка, что именно отправлять преподавателю и какие команды использовать для доступа.

1) Публичный URL

- Пример публичного адреса, который вы указывали: `http://158.160.22.96:8001` — это внешний IP + порт, по которому доступно приложение.
  Это не "ID", а просто адрес и порт сервера. Преподаватель может открыть его в браузере, чтобы посмотреть приложение и Swagger.
