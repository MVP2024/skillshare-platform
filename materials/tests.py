...
class CourseViewSetTest(APITestCase):
    def setUp(self):
        ...
        self.moderator = User.objects.create_user(email="mod@example.com", password="pass")
    ...
    def test_create_course_non_moderator(self):
        ...
        response = self.client.post(reverse('materials:course-list'), {'title': 'Test course'})
    ...
    def test_update_course_owner(self):
        ...
        self.client.force_authenticate(self.user)
        response = self.client.patch(reverse('materials:course-detail', args=[course.id]), {'title': 'Updated'})
        self.assertEqual(response.status_code, 200)

    def test_update_course_moderator(self):
        course = Course.objects.create(title='Course 1', course_user=self.user)
        self.client.force_authenticate(self.moderator)
        response = self.client.patch(reverse('materials:course-detail', args=[course.id]), {'title': 'Updated by mod'})
        self.assertEqual(response.status_code, 200)

    def test_delete_course_owner_allowed(self):
        course = Course.objects.create(title='Course 1', course_user=self.user)
        self.client.force_authenticate(self.user)
        response = self.client.delete(reverse('materials:course-detail', args=[course.id]))
...
class LessonsCRUDTest(APITestCase):
    def setUp(self):
        ...
        self.owner_user = User.objects.create_user(email='owner@example.com', password='testpassword_owner')
        self.moderator_user = User.objects.create_user(email='moderator@example.com', password='testpassword_moderator')
        self.non_owner_user = User.objects.create_user(email='non_owner@example.com', password='testpassword_nonowner')
        self.superuser = User.objects.create_superuser(email='superuser@example.com', password='testpassword_superuser')
        ...
        self.course_owner = Course.objects.create(title='Курс от владельца', course_user=self.owner_user)
        ...
        self.course_non_owner = Course.objects.create(title='Курс не владельца', course_user=self.non_owner_user)
    ...
    def test_lesson_create_by_owner(self, mock_urlopen):  # mock_urlopen передается как аргумент теста
        ...
        mock_urlopen.return_value = mock_response_object  # urlopen должен возвращать этот мок-объект
        mock_urlopen.side_effect = None  # И убедимся, что сам urlopen не выбрасывает исключений

        self.client.force_authenticate(self.owner_user)
        response = self.client.post(self.lessons_list_create_url, self.new_lesson_data, format='json')
    ...
    def test_lesson_update_owner(self):
        ...
        self.client.force_authenticate(self.owner_user)
        response = self.client.patch(self.lesson_detail_url(self.lesson_owner.pk), updated_data)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_lesson_update_non_owner_forbidden(self):
        ...
        self.client.force_authenticate(self.non_owner_user)
        response = self.client.patch(self.lesson_detail_url(self.lesson_owner.pk), updated_data)
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)  # Из-за фильтрации в get_queryset
        self.lesson_owner.refresh_from_db()
    ...
    def test_lesson_delete_moderator_forbidden(self):
        ...
        response = self.client.delete(self.lesson_detail_url(self.lesson_owner.pk))
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
        self.assertTrue(Lesson.objects.filter(pk=self.lesson_owner.pk).exists())  # Урок все еще существует
...
class CourseSubscriptionTest(APITestCase):
    def setUp(self):
        ...
        self.course_owner = User.objects.create_user(email='courseowner@example.com', password='testpass')
        self.course = Course.objects.create(title='Тестовый курс', course_user=self.course_owner)
    ...
    def test_toggle_subscription_invalid_course_id(self):
        ...
        self.assertFalse(CourseSubscription.objects.filter(user=self.user, course__id=invalid_course_id).exists())
...
class ValidatorTest(APITestCase):
    ...
    def test_too_long_url_raises_validation_error(self):
        ...
        long_url = "https://www.youtube.com/watch?v=" + "a" * 1020  # Total length > 1024
        with self.assertRaises(ValidationError) as cm:
    ...
    def test_non_youtube_url_raises_validation_error(self):
        ...
        self.assertIn("Можно использовать только ссылки с YouTube", cm.exception.message)
